...
def add_cart(request, product_id):
    ...
    try:
        cart = Cart.objects.get(cart_id=_cart_id(request)) #get the car using the cart_id present in the session
    ...
    if is_cart_item_exists:
        ...
        if product_variation in ex_var_list:
            #add to the existing cart_item(increase the cart_item quantity)
...
def cart(request, total=0, quantity=0, cart_items=None, tax=0, grand_total=0):
    try:
        ...
        for cart_item in cart_items:
            ...
            quantity = int(quantity) + int(cart_item.quantity)
        tax = (2 * total)/100
